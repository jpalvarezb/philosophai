...
class TripleExtractor:
    ...
    def _build_client(self) -> "OpenAI":
        ...
        client_kwargs: dict[str, object] = {
            "api_key": api_key,
            "base_url": self.config.extraction_api_base,
        }
        if self.config.extraction_http_timeout_seconds is not None:
            client_kwargs["timeout"] = float(
                self.config.extraction_http_timeout_seconds
            )
    ...
    def ensure_schema(self) -> None:
        ...
        con.execute("""
            ...
            """)
        con.execute(
            "CREATE INDEX IF NOT EXISTS idx_normalized_triples_chunk_id ON normalized_triples(chunk_id)"
        )
        con.execute(
            "CREATE INDEX IF NOT EXISTS idx_normalized_triples_text_id ON normalized_triples(text_id)"
        )
        con.execute("""
        ...
        con.execute("""
            ...
            """)
        con.execute(
            "CREATE INDEX IF NOT EXISTS idx_extraction_skips_reason ON extraction_skips(reason)"
        )
        con.execute("""
            ...
            """)
        con.execute(
            "CREATE INDEX IF NOT EXISTS idx_extraction_rejections_chunk_id ON extraction_rejections(chunk_id)"
        )
        con.execute(
            "CREATE INDEX IF NOT EXISTS idx_extraction_rejections_stage ON extraction_rejections(stage)"
        )
    ...
    def fetch_pending_chunks(
        ...
        return [
            ChunkRecord(chunk_id=row[0], text_id=row[1], content=row[2]) for row in rows
        ]
    ...
    def _chunk_skip_reason(content: str) -> str | None:
        ...
        chapter_refs = sum(
            1 for line in sample_lines if CHAPTER_NUMERAL_RE.search(line)
        )
    ...
    @staticmethod
    def _triple_rejection_reason(
        subject_norm: str, predicate_norm: str, object_norm: str
    ) -> str | None:
    ...
    def evaluate_candidates(
        ...
        *,
        ...
        effective_batch_size = max(
            1, int(batch_size or self.config.extraction_judge_batch_size)
        )
    ...
    def process_batch_task(
        ...
        for attempt in range(self.config.extraction_max_retries):
            try:
                ...
                missing_candidates = [
                    ...
                    if candidate.triple_id not in decisions
                ]
                    ...
                    if not decision.keep:
                        ...
                        rejection_rows.append(
                            (
                                ...
                                candidate.object_norm,
                                "llm_judge",
                                (decision.reason or "rejected by evaluator")[
                                    : self.config.max_entity_length
                                ],
                                self.config.judge_model,
                                run_id,
        ...
        if last_error is not None:
            logger.error(
                "Extraction/evaluation batch failed after retries: %s", last_error
            )
            logger.warning(
    ...
    def insert_triples(
        self, triple_rows: list[tuple], entity_rows: list[tuple]
    ) -> None:
    ...
    def _preflight_check(self) -> None:
        ...
        if self.config.judge_model != self.config.extraction_model:
            ...
            logger.info(
                "Preflight: judge_model same as extraction_model — single model check only"
            )
    ...
    def extract_pending(
        ...
        if not chunks:
            return {
                ...
                "rejected_triples": 0,
                "skipped_chunks": 0,
                "entities_tracked": self.storage.con.execute(
        ...
        if not extractable_chunks:
            ...
            return {
                ...
                "entity_types_rows": self.storage.con.execute(
        ...
        batches = [
            ...
            for i in range(
                0, len(extractable_chunks), self.config.extraction_batch_size
            )
        ]
        ...
        with ThreadPoolExecutor(
            max_workers=self.config.extraction_max_workers
        ) as executor:
            futures = {
                executor.submit(self.process_batch_task, batch): batch
            ...
            for future in as_completed(futures):
                ...
                if should_log_progress:
                    ...
                    triples_per_second = (
                        total_triples / elapsed if total_triples else 0.0
                    )
