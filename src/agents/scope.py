...
VALID_AUTHORS = frozenset(
    {
        ...
        "Confucius",
        "David Hume",
        "Djéli Mamoudou Kouyaté (griot tradition)",
        "Dogen",
        "Epictetus",
        "Epicurus",
        "Friedrich Nietzsche",
...
def extract_scope(query: str) -> Scope:
    ...
    for alias, canonical in WORK_ALIASES.items():
        if re.search(
            r"\b(?:in\s+(?:the\s+)?|from\s+(?:the\s+)?)" + re.escape(alias) + r"\b",
            query_lower,
        ):
...
class ScopeFilter:
    ...
    def filter_chunk_ids(self, chunk_ids: list[str]) -> list[str]:
        ...
        rows = self.storage.con.execute(
            sql, list(chunk_ids) + list(text_ids)
        ).fetchall()
    ...
    def derive_communities(
        ...
        """
        chunk_ids = self.get_scoped_chunk_ids()
        return self.storage.derive_scoped_communities(
            chunk_ids, node_to_community, top_n
        )
